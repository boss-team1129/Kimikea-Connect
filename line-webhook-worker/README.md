...
- 注文完了通知で使う `LINE_ORDER_NOTIFY_SECRET` もコードに書きません。
...
注文完了LINE通知用の共有Secretも登録します。任意の長いランダム文字列を1つ決め、Apps Script側にも同じ値を設定します。

```bash
npx wrangler secret put LINE_ORDER_NOTIFY_SECRET
```
...
注文連携用にOrder API URLも通常の環境変数へ追加します。

```text
ORDER_API_URL
https://script.google.com/macros/s/AKfycbyWxvyWZ7_qTkCgvkuim-h_AhDQEHag7xexg8iDqyHE-toaRA6ttQPpNjUaNfTyTYhA/exec
```
...
注文完了LINE通知用の共有Secretも追加します。

```text
LINE_ORDER_NOTIFY_SECRET
```

値にはApps Script側のスクリプトプロパティ `LINE_ORDER_NOTIFY_SECRET` と同じ文字列を入れます。
...
会員はKimikea Connectのマイページで1回限りの連携コードを発行し、LINE公式アカウントへ `連携 ABC123` のように送ります。WorkerがOrder APIへ照会し、該当するKimikea ConnectのuserIdへLINE userIdを保存します。

注文完了通知は、Order APIからWorkerの `/order-notification` へ送ります。Worker URLが次の場合:

```text
https://kimikea-line-webhook.あなたの名前.workers.dev
```

Apps Scriptのスクリプトプロパティには以下を設定します。

```text
LINE_WORKER_NOTIFY_URL
https://kimikea-line-webhook.あなたの名前.workers.dev/order-notification

LINE_ORDER_NOTIFY_SECRET
Cloudflare Secretと同じ長いランダム文字列
```

通知は注文保存と既存メール処理の後に実行されます。LINE通知に失敗しても注文自体は成功扱いのまま、注文一覧の `lineOrderNotifyStatus`、`lineOrderNotifySentAt`、`lineOrderNotifyError`、`lineOrderNotifyTargetUserId` に結果を残します。
